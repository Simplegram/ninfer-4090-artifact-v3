...
std::uint64_t element_count(const Shape& shape) {
    std::uint64_t elements = 1;
    for (const auto dim : shape) { elements = checked_mul(elements, dim, "tensor elements"); }
    return elements;
}
...
Binding parse_binding(const Directory& directory, const Json& value, std::string_view label) {
    ...
    if (value.size() == 1 && value.contains("object")) {
        ...
        out.elements       = element_count(object.shape);
    ...
    for (const auto& part : parts) {
        ...
        if (begin >= end || end > element_count(directory.tensor(handle).shape)) {
...
void parse_uses(Directory& out, const Json& values) {
    ...
    for (const auto& value : values) {
        require_members(value, {"parameter", "input"}, {"activation_policy", "auxiliaries"},
                        "Use");
        Use use;
